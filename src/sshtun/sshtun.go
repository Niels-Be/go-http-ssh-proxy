...
// If localPort is 0 a random port is choosen
...
func (tun *SSHTun) GetLocalPort() int {
...
func (tun *SSHTun) Start() error {
	...
	if tun.local.Port == 0 {
		if addr, ok := localList.Addr().(*net.TCPAddr); ok {
			tun.local.Port = addr.Port
		}
	}

	if tun.debug {
		log.Printf("Connecting SSH to %s", server)
	}
	...
	go func() {
		err := sshConn.Wait()
		if tun.debug {
			log.Printf("SSH connection ended: %s", err.Error())
		}
		tun.errStarted(err)
	}()
	...
	go func() {
		...
		case <-tun.ctx.Done():
			if tun.debug {
				log.Printf("Ctx Done: Closing local port")
			}
			localList.Close()
			sshConn.Close()
		}
	...
	if tun.debug {
		log.Printf("Listening on %s", localList.Addr().String())
	}
	tun.started = true
	if tun.connState != nil {
...
func (tun *SSHTun) Forward(localConn net.Conn) error {
	...
	go func() {
		...
		// } else {
		// 	log.Printf("Done Copy remote->local")
		// }
		myCancel()
	}()
	...
	go func() {
		...
		// } else {
		// 	log.Printf("Done Copy local->remote")
		// }
		myCancel()
	}()
